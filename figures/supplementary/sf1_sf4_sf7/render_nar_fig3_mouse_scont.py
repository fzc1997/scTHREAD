...
TABLES = PROJECT / "source_data" / "tables"
ASE_ROOT = Path(
    os.environ.get(
        "SCTHREAD_ISOFORM_ROOT",
        str(Path(__file__).resolve().parents[3] / "source_data" / "sf4_malat1" / "ase_sources"),
    )
)
...
MOUSE_GENES = Path(
    os.environ.get("SCTHREAD_ISOFORM_ROOT", str(Path(__file__).resolve().parents[3] / "source_data" / "sf4_malat1")) + "/mouse_gene_intervals_grcm39.tsv"
)
...
def _panel_label(ax: plt.Axes, letter: str, x: float = -0.12) -> None:
    letter = letter.upper()  # NAR sets panel letters upper case
    ax.text(
        x,
        1.13,
        letter,
