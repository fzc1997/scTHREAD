...
P0 = REPO / "source_data" / "biological_unit_validation"
...
def replication_text() -> str:
    ...
    return (
        # accessions rather than disease shorthand, as in the panel-e row
        # labels and the arc captions of panels c and d
        f"Targeted DIU: GSE276974 P={ccus.pval:.3g}, Δ={ccus.effect_equal_donor:.2f} · "
        f"GSE307660 P={myeloma.pval:.3g}, Δ={myeloma.effect_equal_donor:.2f}\n"
...
def render(inputs: V3.Inputs, junctions: pd.DataFrame, stem: Path, dpi: int) -> list[Path]:
    ...
    V5.assert_shared_color_ceiling(
        (V5.PRIMARY_WALKTHROUGH, V5.SECONDARY_WALKTHROUGH)
    )
    with plt.rc_context(
        {
            "font.family": S._FAM,
