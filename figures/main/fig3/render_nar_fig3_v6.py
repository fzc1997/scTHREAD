...
Guardrails
    ...
    Both are drawn on one shared 0-5 molecule ceiling, pinned with the portal's
    own colour control, so spatial distribution and colour intensity are
    comparable between them. UMAP is exploratory and does not imply a
    trajectory.
...
JUNCTION_SOURCE = (
    REPO / "source_data" / "portal_screenshots" / "ptprc_views_v6_junction_vector"
)
...
def _draw_cell_type_umap(fig: plt.Figure, caption: str | None = None) -> None:
    V4._panel_heading(
        fig,
        "B",
    ...
    ax = V4._image_axes(
        ...
        V5.PRIMARY_CELL_MAP,
        **V5.canvas_crop(V5.PRIMARY_WALKTHROUGH, "cell"),
    )
    V5._label_relevant_cell_types(ax)
    fig.text(
        0.055,
        0.430,
        caption or f"{V5.DISPLAYED_CELLS:,} sampled cells · shared coordinates",
        fontsize=4.2,
        color=SLATE,
        ha="left",
        va="center",
...
def _draw_isoform_umap(
    ...
    *,
    ...
    V4._image_axes(
        fig,
        [x, 0.445, 0.285, 0.215],
        walkthrough / "02_cell_map.png",
        **V5.canvas_crop(walkthrough, "signal"),
    )
    fig.text(
        x,
        0.430,
        f"{molecules:,} molecules · shared 0–"
        f"{V5.SHARED_COLOR_CEILING:.0f} molecule colour scale",
        fontsize=4.1,
        color=color,
        ha="left",
        va="center",
...
def _draw_umap_guardrail(fig: plt.Figure) -> None:
    fig.text(
        ...
        "Matched coordinates and one shared 0–5 molecule ceiling.",
        fontsize=4.15,
        color=SLATE,
        ha="left",
        va="center",
...
def render(
    ...
    with plt.rc_context(
        ...
        _draw_isoform_umap(
            ...
            letter="C",
            x=0.375,
            walkthrough=V5.PRIMARY_WALKTHROUGH,
            isoform=V5.PRIMARY_ISOFORM,
            molecules=V5.PRIMARY_MOLECULES,
            usage_parts=(
                ("Monocyte", "Monocyte"),
                ("Plasma", "Plasma cell"),
        ...
        _draw_isoform_umap(
            ...
            letter="D",
            x=0.695,
            walkthrough=V5.SECONDARY_WALKTHROUGH,
            isoform=V5.SECONDARY_ISOFORM,
            molecules=V5.SECONDARY_MOLECULES,
            usage_parts=(
                ("Progenitor", "Progenitor"),
                ("B cell", "B cell"),
                ("Monocyte", "Monocyte"),
