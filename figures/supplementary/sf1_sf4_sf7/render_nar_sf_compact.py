...
sys.path.insert(0, str(Path(__file__).resolve().parents[2] / "style"))
sys.path.insert(0, str(Path(__file__).resolve().parents[2] / "main" / "fig2"))
...
def render_sf1():
    ...
    study = R.study_cells()
    atlas = pd.read_csv(A.Path(__file__).resolve().parents[3] / "source_data/tables/fig1_study_atlas.tsv",
                        sep="\t")
    ...
    # Evidence-layer run coverage comes from the frozen release's own study
    # table, which covers every released study (34 rows). It used to be read
    # from a sheet of the supplementary workbook, which stopped shipping when
    # that workbook was cut to four tables, so this reads the source the sheet
    # was built from. The older figdata matrix (ed7_coverage_matrix.tsv) listed
    # only the 15 studies of the former analysis subset and under-reported
    # coverage against the release.
    cov = (
        pd.read_csv(
            A.Path(__file__).resolve().parents[3]
            / "database/frozen_release/release_study_cells.tsv",
            sep="\t", keep_default_na=False,
        )[["gse", "ASE", "DIU", "APA", "F2"]]
        .rename(columns={"F2": "F2jct"})
        .sort_values("gse")
        .reset_index(drop=True)
    )
    # Per-study cells are authoritative in the frozen release; the figdata
    # cohort_cellcount table predates it and is not used here.
    ...
    # The six systems of the Results paragraph. The older grouping in
    # tables/fig1_study_atlas.tsv carried a "Smart-seq2" category, which names a
    # library protocol rather than a biological system, and an "Other" bucket;
    # tables/biological_system_map.tsv assigns all 34 datasets to real biology
    # and is checked below against the totals the text reports.
    ax = fig.add_subplot(gs[1, 0:3])
    # "Development and embryo" is wider than the shared left margin allows.
    _box = ax.get_position()
    ax.set_position((_box.x0 + 0.035, _box.y0, _box.width - 0.035, _box.height))
    tag(ax, "c", x=-0.075); title(ax, "Cells by biological system")
    sysmap = pd.read_csv(
        A.Path(__file__).resolve().parents[3] / "source_data/tables/biological_system_map.tsv",
        sep="\t")
    study = study.drop(columns=["system_display"]).merge(
        sysmap[["gse", "system_display"]], on="gse", how="left")
    unmapped = study.loc[study["system_display"].isna(), "gse"].tolist()
    if unmapped:
        raise RuntimeError(f"datasets with no biological system: {unmapped}")
    sysagg = (study.groupby("system_display")
                   .agg(cells=("cells", "sum"), studies=("gse", "nunique"))
                   .sort_values("cells"))
    if int(sysagg["cells"].sum()) != n_cells or int(sysagg["studies"].sum()) != n_studies:
        raise RuntimeError(
            f"system grouping covers {int(sysagg['cells'].sum()):,} cells / "
            f"{int(sysagg['studies'].sum())} datasets, not the release "
            f"{n_cells:,} / {n_studies}")
    ...
    for yi, (key, row) in enumerate(sysagg.iterrows()):
        n_ds = int(row["studies"])
        ax.text(row["cells"] / 1000 + 5, yi,
                f"{int(row['cells']):,} · {n_ds} dataset" + ("s" if n_ds != 1 else ""),
    ...
    # The portal labels below are far longer than a GEO accession, so this
    # panel is indented past the shared left margin to keep them off the edge.
    _box = ax.get_position()
    _indent = 0.085
    ax.set_position((_box.x0 + _indent, _box.y0, _box.width - _indent, _box.height))
    tag(ax, "e", x=-0.075); title(ax, "Evidence-layer run coverage by study")
    ...
    # Three datasets predate their public accessions and carry internal
    # identifiers (OWN_ASE_scONT, GSA_mouse_testis_11week, benagen_human_ovary)
    # that mean nothing to a reader and appear nowhere on the portal. Label them
    # the way the site does; GEO datasets already show the accession the site
    # shows. Portal wording is carried in tables/biological_system_map.tsv so
    # the figure does not depend on the site being reachable at render time.
    portal = dict(zip(sysmap["gse"], sysmap["portal_label"]))
    ylabels = [g if str(g).startswith("GSE") else (portal.get(g) or g)
               for g in df.gse.values]
    ax.set_yticks(range(n)); ax.set_yticklabels(ylabels, fontsize=4.4)
    for i in range(n):
        for j in range(4):
            v = int(mat[i, j])
            if v > 0:
                ax.text(j, i, str(v), ha="center", va="center", fontsize=4.5,
                        color="white" if mat_n[i, j] > 0.5 else INK, fontweight="bold")
        ax.text(3.66, i, f"{int(df.n_cells.iloc[i]):,} cells",
                va="center", fontsize=4.2, color=SLATE)
    ax.set_xlim(-0.5, 6.0)
