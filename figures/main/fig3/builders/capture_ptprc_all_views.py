...
def _select_with_option(
    ...
    *,
    ...
    """Select the requested option of one named Cell map dropdown."""
    ...
    raise RuntimeError(f"Could not find #{control_id} option {option_text!r}")


def _choose_right_view(page: Page, right_view: str) -> dict[str, object]:
    """Pick the right-panel signal from the Cell map radio group."""
    choice = page.locator(f'input[name="umapSignal"][value="{right_view}"]')
    if choice.count() != 1:
        raise RuntimeError(f"Right-view choice {right_view!r} is not offered")
    if choice.is_disabled():
        raise RuntimeError(
            f"Right-view choice {right_view!r} is unavailable for this gene"
        )
    label = choice.locator("xpath=..")
    label.click()
    if not choice.is_checked():
        raise RuntimeError(f"Right-view choice {right_view!r} did not take effect")
    return {
        "control_id": "umapSignalChoices",
        "selected_value": right_view,
        "selected_text": label.inner_text().strip(),
    }
    ...
    page.wait_for_function(
        """expected => {
            ...
            return state.startsWith("rendered:")
                ...
                && (!expected.source || document.querySelector("#umapStage")?.value === expected.source)
...
def apply_color_max(page: Page, ceiling: float) -> dict[str, object]:
    """Pin the Cell map colour ceiling with the portal's own `Color max` control.

    The portal already shares one auto ceiling across every isoform of a gene,
    so panels only need a common explicit value to be directly comparable.  The
    slider's auto step is derived from that auto ceiling and is usually coarser
    than the requested value, so the step is relaxed before the value is set;
    the applied ceiling is whatever the portal reports back in its legend.
    """
    if page.locator("#umapColorMaxControl").get_attribute("hidden") is not None:
        raise RuntimeError("The Cell map colour-ceiling control is unavailable")
    before = page.locator("#umapColorMax").evaluate(
        "slider => ({min: slider.min, max: slider.max, step: slider.step, value: slider.value})"
    )
    if float(before["max"]) < ceiling:
        raise RuntimeError(
            f"Requested colour ceiling {ceiling} exceeds the portal maximum {before['max']}"
        )
    page.evaluate(
        """ceiling => {
            const slider = document.querySelector("#umapColorMax");
            slider.step = "any";
            slider.value = String(ceiling);
    ...
    page.wait_for_function(
        """ceiling => {
            const shown = document.querySelector("#umapColorMaxValue")?.textContent || "";
            const legend = document.querySelector("#umapSignalLegend")?.textContent || "";
            return shown === new Intl.NumberFormat("en-US").format(ceiling)
                && legend.includes(`0 → ${new Intl.NumberFormat("en-US").format(ceiling)} `);
        }""",
        arg=ceiling,
        timeout=30_000,
    )
    ...
    return {
        ...
        "slider_value": page.locator("#umapColorMax").input_value(),
        "displayed_value": page.locator("#umapColorMaxValue").inner_text().strip(),
        "legend_text": page.locator("#umapSignalLegend").inner_text().strip(),
    }


def canvas_geometry(page: Page) -> dict[str, object]:
    """Record both UMAP canvas boxes in page CSS pixels.

    Downstream figure code crops exactly these boxes out of the screenshot, so
    a label placed at a canvas fraction lands on the same cell it names.
    """
    return {
        role: page.locator(selector).bounding_box()
        for role, selector in (
            ("cell", "#umapCellCanvas"),
            ("signal", "#umapSignalCanvas"),
        )
    }
    ...
    with sync_playwright() as playwright:
        ...
        for view, filename in selected_views:
            ...
            if view == "cellmap":
                ...
                if args.cellmap_color_max is not None:
                    cellmap_config["color_max"] = apply_color_max(
                        page, args.cellmap_color_max
                    )
                cellmap_config["canvas_geometry"] = canvas_geometry(page)
        ...
        metadata = {
            ...
            "cellmap_color_max": args.cellmap_color_max,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Pin the Cell map colour ceiling with the portal's Color max "
            "control so several captures share one comparable scale."
        ),
    )
    parser.add_argument(
        "--metadata-filename",
