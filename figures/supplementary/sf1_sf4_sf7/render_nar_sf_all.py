...
PURPLE = "#6B5B95"
REG = Path(
    os.environ.get("SCTHREAD_SCLONG_ROOT", str(Path(__file__).resolve().parents[3])) + "/source_data/tables/sample_registry_frozen_20260914.tsv"
)
CURRENT_REG = Path(
    os.environ.get("SCTHREAD_SCLONG_ROOT", str(Path(__file__).resolve().parents[3])) + "/source_data/tables/sample_registry_frozen_20260914.tsv"
)
WALK = Path(__file__).resolve().parents[3] / "source_data/portal_screenshots"
WALK_CORRECTED = WALK / "corrected_20260727"
F2 = Path(__file__).resolve().parents[3] / "source_data" / "f2_grammar_figdata"
P0 = Path(__file__).resolve().parents[3] / "source_data/biological_unit_validation"
...
def render_sf1():
    ...
    study = R.study_cells()
    atlas = pd.read_csv(
        Path(__file__).resolve().parents[3] / "source_data/tables/fig1_study_atlas.tsv", sep="\t")
...
def render_sf9(
    ...
    tab = Path(__file__).resolve().parents[3] / "source_data/tables"
